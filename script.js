...
const POWERUP_DEFS = {
  freeze: {
    ...
    desc:'Congela os fantasmas por 8s', dur:480, type:'timed', minLevel:1,
    applyFn() { freezeActive = true; freezeTimer = this.dur; showIG('🧊 Fantasmas congelados!'); },
  },
  speed: {
    id:'speed', icon:'⚡', name:'Turbo', color:'#f7c948',
    desc:'Velocidade dobrada por 10s', dur:600, type:'timed', minLevel:1,
    applyFn() { speedTimer = this.dur; showIG('⚡ Turbo ativado!'); },
  },
  slowMonster: {
    id:'slowMonster', icon:'🐌', name:'Monstro Lento', color:'#a3e635',
    desc:'Fantasmas ficam lentos nesta fase', dur:-1, type:'passive', minLevel:2,
    applyFn() { passivePowers.add('slowMonster'); monsterSpeed = Math.max(monsterSpeed, 130); monster2Speed = Math.max(monster2Speed, 200); showIG('🐌 Monstros lentos!'); },
    removeFn() { passivePowers.delete('slowMonster'); },
  },
  dumbMonster: {
    id:'dumbMonster', icon:'🤖', name:'Monstro Burro', color:'#fb923c',
    desc:'Fantasmas andam aleatório 70% nesta fase', dur:-1, type:'passive', minLevel:3,
  ...
  torch: {
    ...
    desc:'Ilumina o labirinto na névoa (3 usos)', dur:1200, type:'usable', minLevel:3,
    applyFn() { torchCharges = (torchCharges || 0) + 3; updateTorchBtn(); showIG('🔦 Lanterna: +3 usos!'); },
  },
  extraLife: {
    id:'extraLife', icon:'❤️', name:'Vida Extra', color:'#f43f5e',
    desc:'+1 vida agora!', dur:-1, type:'instant', minLevel:2,
    applyFn() { lives = Math.min(lives + 1, 6); document.getElementById('lives-val').textContent = lives; spawnFloat('❤️+1', player.x, player.y); showIG('❤️ Vida extra!'); },
  },
  compass: {
    id:'compass', icon:'🧭', name:'Bússola', color:'#818cf8',
    desc:'Mostra a direção da saída nesta fase', dur:-1, type:'passive', minLevel:1,
  ...
  shield: {
    ...
    desc:'Protege de 1 ataque nesta fase', dur:-1, type:'passive', minLevel:4,
  ...
  magnet: {
    ...
    desc:'Atrai moedas na mesma linha/coluna por 6s', dur:360, type:'timed', minLevel:2,
    applyFn() { magnetTimer = this.dur; showIG('🧲 Imã ativado!'); },
  },
  reveal: {
    id:'reveal', icon:'👁️', name:'Revelar', color:'#34d399',
    desc:'Remove a névoa permanentemente nesta fase', dur:-1, type:'passive', minLevel:3,
  ...
  doubleScore: {
    ...
    desc:'Dobra todos os pontos nesta fase', dur:-1, type:'passive', minLevel:5,
    applyFn() { passivePowers.add('doubleScore'); showIG('✨ Pontos dobrados!'); },
    removeFn() { passivePowers.delete('doubleScore'); },
  },
};

function getEligiblePicks(lvl) {
  return Object.values(POWERUP_DEFS).filter(p => p.minLevel <= lvl);
}

function pickRandomOffers(lvl) {
  const pool = getEligiblePicks(lvl).filter(p => p.id !== 'freeze' && p.id !== 'speed');
...
function tickPowerTimers() {
  ...
  if (torchActive && torchTimer > 0) { torchTimer--; if (torchTimer <= 0) { torchActive = false; showIG('Lanterna apagou!'); } }
}
...
document.getElementById('torch-btn').addEventListener('click', () => {
  ...
  updateTorchBtn(); showIG('🔦 Lanterna ligada!');
});
...
function showPowerPick(onDone) {
  ...
  for (const pow of offers) {
    ...
    card.innerHTML = '<div class="pp-card-icon">' + pow.icon + '</div><div class="pp-card-name">' + pow.name + '</div><div class="pp-card-desc">' + pow.desc + '</div><div class="pp-card-dur">' + durText + '</div>';
...
let freezeItemInLevel = null, speedItemInLevel = null;
...
function initLevel() {
  ...
  freezeItemInLevel = placeRandom([{...player},{...exit},...keys,{...monster}]);
  speedItemInLevel  = placeRandom([{...player},{...exit},...keys,{...monster},freezeItemInLevel]);
...
function draw(){
  ...
  if(freezeItemInLevel){ctx.save();ctx.shadowColor='#38bdf8';ctx.shadowBlur=14*(.7+.3*Math.sin(t*.15));em(freezeItemInLevel.x*CELL+CELL/2,freezeItemInLevel.y*CELL+CELL/2,CELL*.78,'🧊');ctx.restore();}
  if(speedItemInLevel){ctx.save();ctx.shadowColor='#f7c948';ctx.shadowBlur=14*(.7+.3*Math.sin(t*.15));em(speedItemInLevel.x*CELL+CELL/2,speedItemInLevel.y*CELL+CELL/2,CELL*.78,'⚡');ctx.restore();}
  ...
  if(shouldShowFog()){
    const fx=player.x*CELL+CELL/2,fy=player.y*CELL+CELL/2,fr=torchActive?CELL*6:CELL*3.5;
    const fg=ctx.createRadialGradient(fx,fy,fr*.4,fx,fy,fr*1.5);
    fg.addColorStop(0,'rgba(0,0,0,0)');fg.addColorStop(.5,'rgba(0,0,0,0.55)');fg.addColorStop(1,'rgba(0,0,0,0.97)');
...
function afterMove(){
  ...
  if(freezeItemInLevel&&player.x===freezeItemInLevel.x&&player.y===freezeItemInLevel.y){freezeItemInLevel=null;addToInventory('freeze');}
  if(speedItemInLevel&&player.x===speedItemInLevel.x&&player.y===speedItemInLevel.y){speedItemInLevel=null;addToInventory('speed');}
  ...
  if(player.x===exit.x&&player.y===exit.y&&keysCollected>=totalKeys){
    const elapsed=(frame-achLevelStart)/60,starCount=elapsed<30?3:elapsed<60?2:1;
...
function flashSecret(){
  ...
  const pts=30;score+=pts;document.getElementById('score-val').textContent=score;
...
let gtostTimer=null;
function showIG(msg){const t=document.getElementById('gtost');if(!t)return;t.textContent=msg;t.style.opacity='1';clearTimeout(gtostTimer);gtostTimer=setTimeout(()=>{t.style.opacity='0';},2000);}
...
function showOverlay(title,sub,btnText){document.getElementById('ov-title').innerHTML=title;document.getElementById('ov-title').style.color=gameState==='dead'?'#ff4466':'#f7c948';document.getElementById('ov-sub').textContent=sub;document.getElementById('ov-btn').textContent=btnText;document.getElementById('game-overlay').classList.add('show');}
...
const DLGS={
  ...
  3:'Fase 3 — Oceano! Nevoa desceu... e um segundo monstro chegou!',
  4:'Fase 4 — Vulcao! Portais de lava podem te teletransportar!',
  5:'Fase 5 — Gelo! Use os poderes com sabedoria!',
  6:'Fase 6 — Noite Eterna! O desafio maximo começa agora...',
};
let dialogTimer=null;
function showDialog(lvl){const txt=DLGS[((lvl-1)%6)+1];document.getElementById('dialog-text').textContent=txt;document.getElementById('dialog-box').classList.add('show');clearTimeout(dialogTimer);dialogTimer=setTimeout(closeDialog,4000);}
...
function quitToMenu(){document.getElementById('pause-overlay').classList.remove('show');gameState='intro';level=1;score=0;showOverlay('🧳 Labirinto Magico!','Pegue as chaves e corra para a porta!\nO fantasminha vai te pegar!','▶ Jogar!');}
...
function saveScore(){const name=document.getElementById('ranking-name-input').value.trim()||'Anonimo';const r=getRanking();r.push({name,score,level,date:new Date().toLocaleDateString('pt-BR')});r.sort((a,b)=>b.score-a.score);saveRanking(r.slice(0,10));renderRanking();document.getElementById('ranking-name-input').value='';showToast('Pontuacao salva!');}
...
function tutNext(){const steps=document.querySelectorAll('.tut-step');steps[tutStep].classList.remove('active');tutStep++;if(tutStep>=TUT_STEPS){closeTutorial();return;}steps[tutStep].classList.add('active');document.querySelectorAll('.tut-dot').forEach((d,i)=>d.classList.toggle('active',i===tutStep));document.getElementById('tut-next').innerHTML=tutStep===TUT_STEPS-1?'Vamos jogar!':'Proximo →';}
...
function openTutorial(){tutStep=0;document.querySelectorAll('.tut-step').forEach((s,i)=>s.classList.toggle('active',i===0));document.getElementById('tut-next').innerHTML='Proximo →';buildTutDots();document.getElementById('tutorial-overlay').classList.add('show');}
...
const ACHIEVEMENTS=[
  ...
  {id:'freeze_use',icon:'❄️',name:'Cacador Gelido',desc:'Use o poder de gelo'},
  ...
  {id:'coins50',icon:'💰',name:'Rico!',desc:'Colete 50 moedas no total'},
...
function checkAchievements(){if(gameState==='win')unlockAch('first_win');if(level>=5)unlockAch('level5');if(level>=10)unlockAch('level10');if(score>=1000)unlockAch('score1000');if(score>=5000)unlockAch('score5000');if(achSecretCount>=3)unlockAch('secret3');if(!achMaxDanger&&gameState==='win')unlockAch('no_danger');if(gameState==='win'&&(frame-achLevelStart)/60<30)unlockAch('speedrun');if(achCoinTotal>=20)unlockAch('collector');if(achCoinTotal>=50)unlockAch('coins50');if(totalKeys>=3&&keysCollected>=3)unlockAch('keys3');if(fogActive&&gameState==='win')unlockAch('fog_survivor');}
